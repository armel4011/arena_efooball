...
// ════════════════════════════════════════════════════════════════════
// À placer dans : lib/features/splash/splash_router.dart
//
// Gère la navigation post-splash :
//   - Si premier lancement → SplashScreen complet 4.7s
//   - Sinon → Splash court (juste fade-in du logo) ou direct vers main
//
// Détecte le premier lancement via SharedPreferences.
// ════════════════════════════════════════════════════════════════════
...
// import 'package:arena/core/routing/routes.dart';
...
class SplashPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return firstLaunchAsync.when(
      ...
      data: (isFirstLaunch) {
        if (isFirstLaunch) {
          // Splash D cinématique complet 4.7s
          return SplashScreen(
            isAdmin: isAdmin,
            onComplete: () => _navigate(context),
          );
        } else {
          // Splash court (juste 1s pour donner le temps au système d'initialiser)
  ...
  void _navigate(BuildContext context) {
    ...
    // Adapte selon ton routeur (GoRouter, Navigator 1.0, etc.)
...
// ════════════════════════════════════════════════════════════════════
// SPLASH COURT (1 seconde, lancements suivants)
// ════════════════════════════════════════════════════════════════════
...
class _ChevronShortPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(mkPath(180, 340, 260, 580, 420), whitePaint);
    canvas.drawPath(mkPath(340, 480, 260, 580, 420), accentPaint);
    canvas.drawPath(mkPath(480, 620, 260, 580, 420), fainted);
  }
