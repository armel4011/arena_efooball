...
insert into auth.users(id) values
  ('ap000000-0000-0000-0000-0000000000a1'),  -- propriétaire (awaiting)
  ('ap000000-0000-0000-0000-0000000000a2');  -- autre joueur (succeeded)
insert into profiles(id,username,email,country_code,referral_code,role,is_active) values
  ('ap000000-0000-0000-0000-0000000000a1','ap_o','apo@ci.invalid','CM','APOO','player',true),
  ('ap000000-0000-0000-0000-0000000000a2','ap_x','apx@ci.invalid','CM','APXX','player',true);

insert into competitions(id,name,game,format,status,start_date,max_players,registration_fee,registration_currency)
values ('ap000000-0000-0000-0000-0000000000c1','ATTACH','efootball','single_elimination','ongoing',now(),4,1000,'XAF');

insert into payments(id,user_id,competition_id,amount_local,currency,provider,status) values
  ('ap000000-0000-0000-0000-0000000000e1','ap000000-0000-0000-0000-0000000000a1','ap000000-0000-0000-0000-0000000000c1',1000,'XAF','mobile_money_manual','awaiting_admin'),
  ('ap000000-0000-0000-0000-0000000000e2','ap000000-0000-0000-0000-0000000000a2','ap000000-0000-0000-0000-0000000000c1',1000,'XAF','mobile_money_manual','succeeded');
...
set local request.jwt.claims = '{"sub":"ap000000-0000-0000-0000-0000000000a2"}';
do $$ begin
  perform public.attach_payment_proof('ap000000-0000-0000-0000-0000000000e1', 'proofs/hack.jpg');
...
set local request.jwt.claims = '{"sub":"ap000000-0000-0000-0000-0000000000a1"}';
select public.attach_payment_proof('ap000000-0000-0000-0000-0000000000e1', 'proofs/legit.jpg');

-- (3) L'autre joueur joint sur SON paiement mais PAS awaiting_admin (succeeded) → P0002.
set local request.jwt.claims = '{"sub":"ap000000-0000-0000-0000-0000000000a2"}';
do $$ begin
  perform public.attach_payment_proof('ap000000-0000-0000-0000-0000000000e2', 'proofs/late.jpg');
...
select is((select proof_path from payments where id='ap000000-0000-0000-0000-0000000000e1'),
  'proofs/legit.jpg', 'le proprietaire attache bien la preuve sur son paiement awaiting_admin');
select is((select proof_path from payments where id='ap000000-0000-0000-0000-0000000000e2'),
