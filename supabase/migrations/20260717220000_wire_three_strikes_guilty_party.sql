-- =============================================================================
-- ARENA — Câblage de la feature « 3 strikes » (ban à vie au 3e verdict)
-- =============================================================================
-- CONSTAT (audit 2026-07-17). La feature était DORMANTE depuis sa livraison
-- (20260515130002) : `trg_three_strikes_ban` attend une écriture de
-- `disputes.guilty_party_id`, mais AUCUN chemin ne l'écrivait — ni fonction, ni
-- une seule ligne de Dart. En prod : 6 litiges, 0 coupable désigné, 0 banni.
-- L'admin tranchait le litige (tapis vert 3-0) sans jamais pouvoir dire QUI
-- avait triché. Le ban à vie et l'écran /banned + la réintégration sous 48 h
-- étaient donc du code mort.
--
-- Le seul « chemin » qui restait ouvert était l'écriture REST directe — c'est
-- précisément le P0 fermé par 20260717190000. On rend donc au serveur ce que
-- l'attaquant seul pouvait faire.
--
-- DESIGN — la culpabilité est un choix EXPLICITE, jamais une déduction.
-- Perdre un litige ≠ avoir triché : un désaccord de score peut être de bonne
-- foi. `p_guilty_party_id` est donc INDÉPENDANT de `p_winner_id` et vaut NULL
-- par défaut (l'admin tranche le match sans accuser personne). Un ban à vie ne
-- doit jamais tomber en effet de bord d'un arbitrage de score.
--
-- Un `p_cancel` (annulation du match) peut lui aussi désigner un coupable, mais
-- le litige passe alors `cancelled` — et le compteur de strikes ne retient que
-- les litiges `resolved` : une annulation ne fabrique pas de strike.
--
-- ⚠️ SURCHARGE : ajouter un paramètre crée une NOUVELLE signature ; l'ancienne
-- survivrait et PostgREST pourrait router vers elle (cf. le précédent
-- `drop_admin_filter_users_legacy_overload`, 20260523110000). On DROP donc
-- explicitement l'ancienne, et on restaure ses grants à l'identique
-- (postgres/authenticated/service_role — jamais anon).
-- =============================================================================
-- Depends on: 20260714140000 (dernière définition), 20260515130002 (trigger),
--   20260717190000 (guard : la colonne est réservée au serveur).
-- =============================================================================

drop function if exists public.resolve_dispute(uuid, uuid, text, boolean, uuid, integer, integer);
...
comment on function public.resolve_dispute(uuid, uuid, text, boolean, uuid, integer, integer, uuid) is
  ...
  '(trg_three_strikes_ban). Perdre un litige n''est PAS etre coupable : le '
  'choix est explicite et independant de p_winner_id.';
