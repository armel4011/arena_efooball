...
Couverture actuelle (91 tests) : modèles freezed, widgets partagés,
...
(`MainLayout` + `HomePage`), et la phase 4 (`CompetitionsListPage`
filtre + cards, `CompetitionDetailPage` tabs + CTA, `BracketView`
groupage par round, `GroupStandingsView` DataTable). Le happy-path
complet d'invitation admin sera couvert via test d'intégration une
fois l'Edge Function `register-admin` livrée (PHASE 12.5).
